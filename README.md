...
- `Proyek_Analisis_Data.ipynb`: File Interactive Python Notebook (`.ipynb`) tempat dilakukan seluruh proses analisis data.
...
2. Setelah masuk ke dalam root directory (direktori awal) dari proyek ini di komputer lokal Anda, buat virtual environment (contoh dengan nama `.venv`) untuk meng-install seluruh dependency yang dibutuhkan dalam menjalankan notebook dan dashboard.
   ```commandline
   python -m venv .venv
   ```

3. Aktifkan virtual environment `.venv` dengan menjalankan perintah berikut.
   ```commandline
   .\.venv\Scripts\activate
   ```

4. Setelah virtual environment aktif maka langkah selanjutnya adalah menginstall dependency proyek dengan menjalankan perintah berikut.
   ```commandline
...
## Cara Alternatif
Jika langkah nomor 4 dan 5 tidak berhasil dilakukan maka gunakan cara memanggil virtual environment secara manual seperti berikut.
